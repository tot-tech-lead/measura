...
export default function CreateService() {
  ...
  const saveHandler = useCallback(() => {
    ...
    if (conditionsValidationPattern.length > 0) {
      alert(
        `Перевірте чи заповнені умови у наступних полях: ${conditionsValidationPattern
          .map(item => item + 1)
          .join(',')} (кожна умова повинна містити більше ніж 2 символи)`
      );
      return;
    }
          ...
          <View style={styles.conditionsInputs}>
            {conditions.map((item, index) => (
              <View style={styles.conditionsInput} key={'input' + index}>
                ...
                />
                <TouchableOpacity
                  style={styles.conditionsInputRemove}
