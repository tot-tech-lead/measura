...
export default function Stage2({data, setProperty}) {
    return (
        <View style={styles.container}>
            ...
            <UnderlinedInput value={data?.tileWidth ? String(data?.tileWidth) : ""}
                             setValue={(v) => setProperty("tileWidth", v)}
                             label={"Ширина плитки (мм.)"}
                             inputType="numeric" />
            <UnderlinedInput value={data?.tileHeight ? String(data?.tileHeight) : ""}
                             setValue={(v) => setProperty("tileHeight", v)}
                             label={"Довжина плитки (мм.)"}
                             inputType="numeric" />
            <UnderlinedInput value={data?.tileCostForMeterSq ? String(data?.tileCostForMeterSq) : ""}
                             setValue={(v) => setProperty("tileCostForMeterSq", v)}
                             label={"Вартість за 1 м²"}
                             inputType="numeric" />
            <UnderlinedInput value={data?.gluePrice ? String(data?.gluePrice) : ""}
                             setValue={(v) => setProperty("gluePrice", v)}
                             label={"Вартість мішка клею"}
                             inputType="numeric" />
            <UnderlinedInput value={data?.glueWeight ? String(data?.glueWeight) : ""}
