...
export default function Stage3({ data, setProperty }) {
  ...
  let getTarifNameNyId = useCallback(
    id => {
      return (
        services.find(item => item.id === id)?.conditions.join(' + ') || ''
      );
    },
    [services]
  );
          ...
          additionalServices.map(item => (
            <Checkbox
              ...
              onChange={() => {
                ...
                else setSelectedServices(prev => [...prev, item.id]);
              }}
            />
          ))}
      </View>
          ...
          <View style={styles.modalContent}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Txt style={styles.optionText}>{item}</Txt>
                </TouchableOpacity>
              )}
            />
            <TouchableOpacity
